...
#define _alloc_data_for_s(type, _type, size, items, ptr)                                                               \
    do {                                                                                                               \
        lcb_size_t ii;                                                                                                 \
                                                                                                                       \
...
cb_params_touch_parse_arguments(struct cb_params_st *params, int argc, VALUE argv)
{
    ...
    if (argc == 1) {
        ...
        case T_HASH:
            ...
            rb_hash_foreach(keys, cb_params_touch_extract_keys_i, (VALUE)params);
            break;
        default:
            /* single key */
...
cb_params_remove_parse_arguments(struct cb_params_st *params, int argc, VALUE argv)
{
    ...
    if (argc == 1) {
        ...
        case T_HASH:
            ...
            rb_hash_foreach(keys, cb_params_remove_extract_keys_i, (VALUE)params);
            break;
        default:
            /* single key */
                          ...
                          lcb_uint32_t flags, lcb_cas_t cas, lcb_time_t exptime)
{
    ...
    if (rb_obj_is_kind_of(value_obj, rb_eStandardError)) {
        ...
        VALUE msg = rb_funcall(rb_mKernel, cb_id_sprintf, 3, rb_str_new2("unable to convert value for key \"%s\": %s"),
                               key_obj, exc_str);
    ...
    if (TYPE(value_obj) != T_STRING) {
        ...
        rb_raise(cb_eValueFormatError, "unable to convert value for key \"%s\" to string: %s", RSTRING_PTR(key_obj),
                 RSTRING_PTR(val));
    }
    rb_ary_push(params->ensurance, key_obj);
...
{
    struct cb_params_st *params = (struct cb_params_st *)arg;
    cb_params_store_init_item(params, params->idx++, key, value, params->cmd.store.flags, 0, params->cmd.store.ttl);
    return ST_CONTINUE;
}
...
cb_params_store_parse_arguments(struct cb_params_st *params, int argc, VALUE argv)
{
    ...
    case 1:
        ...
        case T_HASH:
            ...
            rb_hash_foreach(keys, cb_params_store_extract_keys_i, (VALUE)params);
            break;
        default:
    ...
    case 2:
        ...
        cb_params_store_init_item(params, 0, rb_ary_entry(argv, 0), rb_ary_entry(argv, 1), params->cmd.store.flags,
                                  params->cmd.store.cas, params->cmd.store.ttl);
        break;
    default:
