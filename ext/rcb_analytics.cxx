...
cb_Backend_analytics_get_pending_mutations(VALUE self, VALUE options)
{
    ...
    try {
        ...
        auto promise = std::make_shared<std::promise<core::operations::management::analytics_get_pending_mutations_response>>();
...
cb_Backend_analytics_dataset_get_all(VALUE self, VALUE options)
{
    ...
    try {
        ...
        cb_extract_timeout(req, options);
        auto promise = std::make_shared<std::promise<core::operations::management::analytics_dataset_get_all_response>>();
        auto f = promise->get_future();
        cluster->execute(req, [promise](auto&& resp) {
            promise->set_value(std::forward<decltype(resp)>(resp));
        });
        ...
        if (resp.ctx.ec) {
            if (resp.errors.empty()) {
                ...
                const auto& first_error = resp.errors.front();
                cb_throw_error(resp.ctx, fmt::format("unable to fetch all datasets ({}: {})", first_error.code, first_error.message));
            }
        }
...
cb_Backend_analytics_dataset_drop(VALUE self, VALUE dataset_name, VALUE options)
{
    ...
    try {
        ...
        auto promise = std::make_shared<std::promise<core::operations::management::analytics_dataset_drop_response>>();
        auto f = promise->get_future();
        cluster->execute(req, [promise](auto&& resp) {
            promise->set_value(std::forward<decltype(resp)>(resp));
        });
        ...
        if (auto resp = cb_wait_for_future(f); resp.ctx.ec) {
            if (resp.errors.empty()) {
                cb_throw_error(resp.ctx, fmt::format("unable to drop dataset `{}`.`{}`", req.dataverse_name, req.dataset_name));
            } else {
                const auto& first_error = resp.errors.front();
...
cb_Backend_analytics_dataset_create(VALUE self, VALUE dataset_name, VALUE bucket_name, VALUE options)
{
    ...
    try {
        ...
        auto promise = std::make_shared<std::promise<core::operations::management::analytics_dataset_create_response>>();
        auto f = promise->get_future();
        cluster->execute(req, [promise](auto&& resp) {
            promise->set_value(std::forward<decltype(resp)>(resp));
        });
        ...
        if (auto resp = cb_wait_for_future(f); resp.ctx.ec) {
            if (resp.errors.empty()) {
                cb_throw_error(resp.ctx, fmt::format("unable to create dataset `{}`.`{}`", req.dataverse_name, req.dataset_name));
            } else {
                const auto& first_error = resp.errors.front();
                cb_throw_error(resp.ctx,
...
cb_Backend_analytics_dataverse_drop(VALUE self, VALUE dataverse_name, VALUE options)
{
    ...
    try {
        ...
        req.dataverse_name = cb_string_new(dataverse_name);
        cb_extract_option_bool(req.ignore_if_does_not_exist, options, "ignore_if_does_not_exist");
        auto promise = std::make_shared<std::promise<core::operations::management::analytics_dataverse_drop_response>>();
        auto f = promise->get_future();
        cluster->execute(req, [promise](auto&& resp) {
            promise->set_value(std::forward<decltype(resp)>(resp));
        });
...
cb_Backend_analytics_dataverse_create(VALUE self, VALUE dataverse_name, VALUE options)
{
    ...
    try {
        ...
        req.dataverse_name = cb_string_new(dataverse_name);
        cb_extract_option_bool(req.ignore_if_exists, options, "ignore_if_exists");
        auto promise = std::make_shared<std::promise<core::operations::management::analytics_dataverse_create_response>>();
        auto f = promise->get_future();
        cluster->execute(req, [promise](auto&& resp) {
            promise->set_value(std::forward<decltype(resp)>(resp));
        });
        if (auto resp = cb_wait_for_future(f); resp.ctx.ec) {
            if (resp.errors.empty()) {
                cb_throw_error(resp.ctx, fmt::format("unable to create dataverse `{}`", req.dataverse_name));
            } else {
                const auto& first_error = resp.errors.front();
                cb_throw_error(
                  resp.ctx,
...
cb_Backend_analytics_index_get_all(VALUE self, VALUE options)
{
    ...
    try {
        ...
        cb_extract_timeout(req, options);
        auto promise = std::make_shared<std::promise<core::operations::management::analytics_index_get_all_response>>();
        auto f = promise->get_future();
        cluster->execute(req, [promise](auto&& resp) {
            promise->set_value(std::forward<decltype(resp)>(resp));
        });
        ...
        if (resp.ctx.ec) {
            if (resp.errors.empty()) {
                ...
                const auto& first_error = resp.errors.front();
                cb_throw_error(resp.ctx, fmt::format("unable to fetch all indexes ({}: {})", first_error.code, first_error.message));
            }
        }
...
cb_Backend_analytics_index_create(VALUE self, VALUE index_name, VALUE dataset_name, VALUE fields, VALUE options)
{
    ...
    try {
        ...
        cb_extract_option_string(req.dataverse_name, options, "dataverse_name");
        cb_extract_option_bool(req.ignore_if_exists, options, "ignore_if_exists");
        auto promise = std::make_shared<std::promise<core::operations::management::analytics_index_create_response>>();
        auto f = promise->get_future();
        cluster->execute(req, [promise](auto&& resp) {
            promise->set_value(std::forward<decltype(resp)>(resp));
        });
...
cb_Backend_analytics_index_drop(VALUE self, VALUE index_name, VALUE dataset_name, VALUE options)
{
    ...
    try {
        ...
        cb_extract_option_string(req.dataverse_name, options, "dataverse_name");
        cb_extract_option_bool(req.ignore_if_does_not_exist, options, "ignore_if_does_not_exist");
        auto promise = std::make_shared<std::promise<core::operations::management::analytics_index_drop_response>>();
        auto f = promise->get_future();
        cluster->execute(req, [promise](auto&& resp) {
            promise->set_value(std::forward<decltype(resp)>(resp));
        });
        if (auto resp = cb_wait_for_future(f); resp.ctx.ec) {
            if (resp.errors.empty()) {
                cb_throw_error(resp.ctx,
                               fmt::format("unable to drop index `{}`.`{}`.`{}`", req.dataverse_name, req.dataset_name, req.index_name));
            } else {
                const auto& first_error = resp.errors.front();
                cb_throw_error(resp.ctx,
...
cb_Backend_analytics_link_connect(VALUE self, VALUE options)
{
    ...
    try {
        ...
        auto promise = std::make_shared<std::promise<core::operations::management::analytics_link_connect_response>>();
        auto f = promise->get_future();
        cluster->execute(req, [promise](auto&& resp) {
            promise->set_value(std::forward<decltype(resp)>(resp));
        });
        ...
        if (auto resp = cb_wait_for_future(f); resp.ctx.ec) {
            if (resp.errors.empty()) {
                cb_throw_error(resp.ctx, fmt::format("unable to connect link `{}` on `{}`", req.link_name, req.dataverse_name));
            } else {
                const auto& first_error = resp.errors.front();
                cb_throw_error(resp.ctx,
...
cb_Backend_analytics_link_disconnect(VALUE self, VALUE options)
{
    ...
    try {
        ...
        cb_extract_option_string(req.link_name, options, "link_name");
        cb_extract_option_string(req.dataverse_name, options, "dataverse_name");
        auto promise = std::make_shared<std::promise<core::operations::management::analytics_link_disconnect_response>>();
        auto f = promise->get_future();
        cluster->execute(req, [promise](auto&& resp) {
            promise->set_value(std::forward<decltype(resp)>(resp));
        });
        if (auto resp = cb_wait_for_future(f); resp.ctx.ec) {
            if (resp.errors.empty()) {
                cb_throw_error(resp.ctx, fmt::format("unable to disconnect link `{}` on `{}`", req.link_name, req.dataverse_name));
            } else {
                const auto& first_error = resp.errors.front();
                cb_throw_error(resp.ctx,
...
cb_Backend_analytics_link_create(VALUE self, VALUE link, VALUE options)
{
    ...
    try {
        ...
        if (ID type = rb_sym2id(link_type); type == rb_intern("couchbase")) {
            core::operations::management::analytics_link_create_request<core::management::analytics::couchbase_remote_link> req{};
            cb_extract_timeout(req, options);
            cb_fill_link(req.link, link);

            auto promise = std::make_shared<std::promise<core::operations::management::analytics_link_create_response>>();
            auto f = promise->get_future();
            cluster->execute(req, [promise](auto&& resp) {
                promise->set_value(std::forward<decltype(resp)>(resp));
            });
            ...
            core::operations::management::analytics_link_create_request<core::management::analytics::azure_blob_external_link> req{};
            cb_extract_timeout(req, options);
            cb_fill_link(req.link, link);
            ...
            core::operations::management::analytics_link_create_request<core::management::analytics::s3_external_link> req{};
            cb_extract_timeout(req, options);
            cb_fill_link(req.link, link);
            ...
            if (auto resp = cb_wait_for_future(f); resp.ctx.ec) {
                if (resp.errors.empty()) {
                    cb_throw_error(resp.ctx,
                                   fmt::format("unable to create s3_external link `{}` on `{}`", req.link.link_name, req.link.dataverse));
                } else {
                    const auto& first_error = resp.errors.front();
                    cb_throw_error(resp.ctx,
...
cb_Backend_analytics_link_replace(VALUE self, VALUE link, VALUE options)
{
    ...
    try {
        ...
        if (ID type = rb_sym2id(link_type); type == rb_intern("couchbase")) {
            core::operations::management::analytics_link_replace_request<core::management::analytics::couchbase_remote_link> req{};
            cb_extract_timeout(req, options);
            cb_fill_link(req.link, link);

            auto promise = std::make_shared<std::promise<core::operations::management::analytics_link_replace_response>>();
            auto f = promise->get_future();
            cluster->execute(req, [promise](auto&& resp) {
                promise->set_value(std::forward<decltype(resp)>(resp));
            });
            ...
            core::operations::management::analytics_link_replace_request<core::management::analytics::azure_blob_external_link> req{};
            cb_extract_timeout(req, options);
            cb_fill_link(req.link, link);
            ...
            core::operations::management::analytics_link_replace_request<core::management::analytics::s3_external_link> req{};
            cb_extract_timeout(req, options);
            cb_fill_link(req.link, link);
            ...
            if (auto resp = cb_wait_for_future(f); resp.ctx.ec) {
                if (resp.errors.empty()) {
                    cb_throw_error(resp.ctx,
                                   fmt::format("unable to replace s3_external link `{}` on `{}`", req.link.link_name, req.link.dataverse));
                } else {
                    const auto& first_error = resp.errors.front();
                    cb_throw_error(resp.ctx,
...
cb_Backend_analytics_link_drop(VALUE self, VALUE link, VALUE dataverse, VALUE options)
{
    ...
    try {
        ...
        auto promise = std::make_shared<std::promise<core::operations::management::analytics_link_drop_response>>();
        auto f = promise->get_future();
        cluster->execute(req, [promise](auto&& resp) {
            promise->set_value(std::forward<decltype(resp)>(resp));
        });
        ...
        if (auto resp = cb_wait_for_future(f); resp.ctx.ec) {
            if (resp.errors.empty()) {
                cb_throw_error(resp.ctx, fmt::format("unable to drop link `{}` on `{}`", req.link_name, req.dataverse_name));
            } else {
                const auto& first_error = resp.errors.front();
                cb_throw_error(
                  resp.ctx,
...
cb_Backend_analytics_link_get_all(VALUE self, VALUE options)
{
    ...
    try {
        ...
        auto promise = std::make_shared<std::promise<core::operations::management::analytics_link_get_all_response>>();
        auto f = promise->get_future();
        cluster->execute(req, [promise](auto&& resp) {
            promise->set_value(std::forward<decltype(resp)>(resp));
        });
...
cb_Backend_document_analytics(VALUE self, VALUE statement, VALUE options)
{
    ...
    try {
        ...
        if (VALUE named_params = rb_hash_aref(options, rb_id2sym(rb_intern("named_parameters"))); !NIL_P(named_params)) {
            ...
            // NOLINTNEXTLINE(cppcoreguidelines-pro-type-reinterpret-cast)
            rb_hash_foreach(named_params, cb_for_each_named_param_analytics, reinterpret_cast<VALUE>(&req));
        }
        ...
        if (resp.ctx.ec) {
            if (!resp.meta.errors.empty()) {
                ...
                cb_throw_error(resp.ctx, fmt::format("unable to execute analytics query ({}: {})", first_error.code, first_error.message));
            } else {
...
init_analytics(VALUE cBackend)
{
    ...
    // Management APIs
    rb_define_method(cBackend, "analytics_get_pending_mutations", cb_Backend_analytics_get_pending_mutations, 1);
    rb_define_method(cBackend, "analytics_dataverse_drop", cb_Backend_analytics_dataverse_drop, 2);
    rb_define_method(cBackend, "analytics_dataverse_create", cb_Backend_analytics_dataverse_create, 2);
