    ...
    switch (TYPE(val)) {
      ...
      default:
        throw ruby_exception(
          rb_eArgError,
          rb_sprintf(
            "%s must be a Integer representing milliseconds, but given %+" PRIsVALUE, name, val));
    }
  }
}
...
cb_Backend_open(VALUE self, VALUE connection_string, VALUE credentials, VALUE options)
{
  ...
  try {
    ...
    if (NIL_P(certificate_path) || NIL_P(key_path)) {
      ...
      if (!NIL_P(options)) {
        VALUE allowed_mechanisms =
          rb_hash_aref(options, rb_id2sym(rb_intern("allowed_sasl_mechanisms")));
        if (!NIL_P(allowed_mechanisms)) {
          ...
          if (allowed_mechanisms_size < 1) {
            throw ruby_exception(exc_invalid_argument(),
                                 "allowed_sasl_mechanisms list cannot be empty");
          }
      ...
      if (!connstr.tls) {
        throw ruby_exception(exc_invalid_argument(),
                             "Certificate authenticator requires TLS connection, check the schema "
    ...
    if (origin.options().enable_tracing) {
      cb_extract_option_milliseconds(
        origin.options().tracing_options.orphaned_emit_interval, options, "orphaned_emit_interval");
      cb_extract_option_number(
        origin.options().tracing_options.orphaned_sample_size, options, "orphaned_sample_size");
      cb_extract_option_milliseconds(origin.options().tracing_options.threshold_emit_interval,
                                     options,
                                     "threshold_emit_interval");
      cb_extract_option_number(
        origin.options().tracing_options.threshold_sample_size, options, "threshold_sample_size");
      cb_extract_option_milliseconds(
        origin.options().tracing_options.key_value_threshold, options, "key_value_threshold");
      cb_extract_option_milliseconds(
        origin.options().tracing_options.query_threshold, options, "query_threshold");
      cb_extract_option_milliseconds(
        origin.options().tracing_options.view_threshold, options, "view_threshold");
      cb_extract_option_milliseconds(
        origin.options().tracing_options.search_threshold, options, "search_threshold");
      cb_extract_option_milliseconds(
        origin.options().tracing_options.analytics_threshold, options, "analytics_threshold");
      cb_extract_option_milliseconds(
        origin.options().tracing_options.management_threshold, options, "management_threshold");
    }
    cb_extract_option_bool(origin.options().enable_metrics, options, "enable_metrics");
    if (origin.options().enable_metrics) {
      cb_extract_option_milliseconds(
        origin.options().metrics_options.emit_interval, options, "metrics_emit_interval");
    }
    cb_extract_option_milliseconds(
      origin.options().bootstrap_timeout, options, "bootstrap_timeout");
    ...
    cb_extract_option_milliseconds(
      origin.options().key_value_timeout, options, "key_value_timeout");
    cb_extract_option_milliseconds(
      origin.options().key_value_durable_timeout, options, "key_value_durable_timeout");
    ...
    cb_extract_option_milliseconds(
      origin.options().analytics_timeout, options, "analytics_timeout");
    cb_extract_option_milliseconds(origin.options().search_timeout, options, "search_timeout");
    cb_extract_option_milliseconds(
      origin.options().management_timeout, options, "management_timeout");
    cb_extract_option_milliseconds(
      origin.options().tcp_keep_alive_interval, options, "tcp_keep_alive_interval");
    cb_extract_option_milliseconds(
      origin.options().config_poll_interval, options, "config_poll_interval");
    cb_extract_option_milliseconds(
      origin.options().config_poll_floor, options, "config_poll_floor");
    cb_extract_option_milliseconds(
      origin.options().config_idle_redial_timeout, options, "config_idle_redial_timeout");
    cb_extract_option_milliseconds(
      origin.options().idle_http_connection_timeout, options, "idle_http_connection_timeout");

    cb_extract_dns_config(origin.options().dns_config, options);

    cb_extract_option_number(
      origin.options().max_http_connections, options, "max_http_connections");

    cb_extract_option_bool(origin.options().enable_tls, options, "enable_tls");
    cb_extract_option_bool(
      origin.options().enable_mutation_tokens, options, "enable_mutation_tokens");
    cb_extract_option_bool(
      origin.options().enable_tcp_keep_alive, options, "enable_tcp_keep_alive");
    ...
    cb_extract_option_bool(
      origin.options().enable_unordered_execution, options, "enable_unordered_execution");
    cb_extract_option_bool(
      origin.options().enable_clustermap_notification, options, "enable_clustermap_notification");
    ...
    if (auto ec = cb_wait_for_future(f)) {
      cb_throw_error_code(ec,
                          fmt::format("unable open cluster at {}", origin.next_address().first));
    }
