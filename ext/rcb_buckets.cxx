...
{
void
cb_generate_bucket_settings(VALUE bucket, core::management::cluster::bucket_settings& entry, bool is_create)
{
    ...
    if (VALUE eviction_policy = rb_hash_aref(bucket, rb_id2sym(rb_intern("eviction_policy"))); !NIL_P(eviction_policy)) {
        if (TYPE(eviction_policy) == T_SYMBOL) {
            if (eviction_policy == rb_id2sym(rb_intern("full"))) {
                ...
                entry.eviction_policy = core::management::cluster::bucket_eviction_policy::not_recently_used;
            } else {
        ...
        if (VALUE conflict_resolution_type = rb_hash_aref(bucket, rb_id2sym(rb_intern("conflict_resolution_type")));
            ...
            if (TYPE(conflict_resolution_type) == T_SYMBOL) {
                if (conflict_resolution_type == rb_id2sym(rb_intern("timestamp"))) {
                    entry.conflict_resolution_type = core::management::cluster::bucket_conflict_resolution::timestamp;
                } else if (conflict_resolution_type == rb_id2sym(rb_intern("sequence_number"))) {
                    entry.conflict_resolution_type = core::management::cluster::bucket_conflict_resolution::sequence_number;
                } else if (conflict_resolution_type == rb_id2sym(rb_intern("custom"))) {
                    entry.conflict_resolution_type = core::management::cluster::bucket_conflict_resolution::custom;
                } else {
                    throw ruby_exception(rb_eArgError,
...
cb_Backend_bucket_create(VALUE self, VALUE bucket_settings, VALUE options)
{
    ...
    try {
        ...
        auto promise = std::make_shared<std::promise<core::operations::management::bucket_create_response>>();
...
cb_Backend_bucket_update(VALUE self, VALUE bucket_settings, VALUE options)
{
    ...
    try {
        ...
        auto promise = std::make_shared<std::promise<core::operations::management::bucket_update_response>>();
        auto f = promise->get_future();
...
cb_Backend_bucket_drop(VALUE self, VALUE bucket_name, VALUE options)
{
    ...
    try {
        ...
        cb_extract_timeout(req, options);
        auto promise = std::make_shared<std::promise<core::operations::management::bucket_drop_response>>();
        auto f = promise->get_future();
        cluster->execute(req, [promise](core::operations::management::bucket_drop_response&& resp) {
            promise->set_value(std::move(resp));
        });
        if (auto resp = cb_wait_for_future(f); resp.ctx.ec) {
            cb_throw_error(resp.ctx, fmt::format("unable to remove bucket \"{}\" on the cluster", req.name));
        }
        return Qtrue;
    } catch (const std::system_error& se) {
        rb_exc_raise(cb_map_error_code(se.code(), fmt::format("failed to perform {}: {}", __func__, se.what()), false));
...
cb_Backend_bucket_flush(VALUE self, VALUE bucket_name, VALUE options)
{
    ...
    try {
        ...
        cb_extract_timeout(req, options);
        auto promise = std::make_shared<std::promise<core::operations::management::bucket_flush_response>>();
        auto f = promise->get_future();
        cluster->execute(req, [promise](core::operations::management::bucket_flush_response&& resp) {
            promise->set_value(std::move(resp));
        });
        if (auto resp = cb_wait_for_future(f); resp.ctx.ec) {
            cb_throw_error(resp.ctx, fmt::format("unable to flush bucket \"{}\" on the cluster", req.name));
        }
...
cb_Backend_bucket_get_all(VALUE self, VALUE options)
{
    ...
    try {
        ...
        cb_extract_timeout(req, options);
        auto promise = std::make_shared<std::promise<core::operations::management::bucket_get_all_response>>();
        auto f = promise->get_future();
...
cb_Backend_bucket_get(VALUE self, VALUE bucket_name, VALUE options)
{
    ...
    try {
        ...
        cb_extract_timeout(req, options);
        auto promise = std::make_shared<std::promise<core::operations::management::bucket_get_response>>();
        auto f = promise->get_future();
        ...
        auto resp = cb_wait_for_future(f);
        if (resp.ctx.ec) {
            cb_throw_error(resp.ctx, fmt::format("unable to locate bucket \"{}\" on the cluster", req.name));
        }
