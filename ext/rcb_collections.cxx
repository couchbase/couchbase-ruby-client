...
cb_Backend_scope_get_all(VALUE self, VALUE bucket_name, VALUE options)
{
    ...
    try {
        ...
        auto promise = std::make_shared<std::promise<core::operations::management::scope_get_all_response>>();
        ...
        if (resp.ctx.ec) {
            cb_throw_error(resp.ctx, fmt::format("unable to get list of the scopes of the bucket \"{}\"", req.bucket_name));
        }
...
cb_Backend_scope_create(VALUE self, VALUE bucket_name, VALUE scope_name, VALUE options)
{
    ...
    try {
        core::operations::management::scope_create_request req{ cb_string_new(bucket_name), cb_string_new(scope_name) };
        cb_extract_timeout(req, options);
        auto promise = std::make_shared<std::promise<core::operations::management::scope_create_response>>();
        auto f = promise->get_future();
        cluster->execute(req, [promise](auto&& resp) {
            promise->set_value(std::forward<decltype(resp)>(resp));
        });
        ...
        if (resp.ctx.ec) {
            cb_throw_error(resp.ctx, fmt::format(R"(unable to create the scope "{}" on the bucket "{}")", req.scope_name, req.bucket_name));
        }
...
cb_Backend_scope_drop(VALUE self, VALUE bucket_name, VALUE scope_name, VALUE options)
{
    ...
    try {
        core::operations::management::scope_drop_request req{ cb_string_new(bucket_name), cb_string_new(scope_name) };
        cb_extract_timeout(req, options);
        auto promise = std::make_shared<std::promise<core::operations::management::scope_drop_response>>();
        auto f = promise->get_future();
        cluster->execute(req, [promise](auto&& resp) {
            promise->set_value(std::forward<decltype(resp)>(resp));
        });
        auto resp = cb_wait_for_future(f);
        if (resp.ctx.ec) {
            cb_throw_error(resp.ctx, fmt::format(R"(unable to drop the scope "{}" on the bucket "{}")", req.scope_name, req.bucket_name));
        }
        return ULL2NUM(resp.uid);
    } catch (const std::system_error& se) {
        rb_exc_raise(cb_map_error_code(se.code(), fmt::format("failed to perform {}: {}", __func__, se.what()), false));
...
cb_Backend_collection_create(VALUE self, VALUE bucket_name, VALUE scope_name, VALUE collection_name, VALUE settings, VALUE options)
{
    ...
    try {
        ...
        auto promise = std::make_shared<std::promise<core::operations::management::collection_create_response>>();
        auto f = promise->get_future();
        cluster->execute(req, [promise](auto&& resp) {
            promise->set_value(std::forward<decltype(resp)>(resp));
        });
...
cb_Backend_collection_update(VALUE self, VALUE bucket_name, VALUE scope_name, VALUE collection_name, VALUE settings, VALUE options)
{
    ...
    try {
        ...
        auto promise = std::make_shared<std::promise<core::operations::management::collection_update_response>>();
        auto f = promise->get_future();
        cluster->execute(req, [promise](auto&& resp) {
            promise->set_value(std::forward<decltype(resp)>(resp));
        });
...
cb_Backend_collection_drop(VALUE self, VALUE bucket_name, VALUE scope_name, VALUE collection_name, VALUE options)
{
    ...
    try {
        ...
        auto promise = std::make_shared<std::promise<core::operations::management::collection_drop_response>>();
        auto f = promise->get_future();
        cluster->execute(req, [promise](auto&& resp) {
            promise->set_value(std::forward<decltype(resp)>(resp));
        });
