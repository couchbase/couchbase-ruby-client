...
cb_Backend_document_lookup_in(VALUE self, VALUE bucket, VALUE scope, VALUE collection, VALUE id, VALUE specs, VALUE options)
{
    ...
    try {
        ...
        for (std::size_t i = 0; i < entries_size; ++i) {
            ...
            req.specs.emplace_back(
              core::impl::subdoc::command{ opcode, cb_string_new(path), {}, core::impl::subdoc::build_lookup_in_path_flags(xattr) });
        }
...
cb_Backend_document_lookup_in_any_replica(VALUE self, VALUE bucket, VALUE scope, VALUE collection, VALUE id, VALUE specs, VALUE options)
{
    ...
    try {
        ...
        auto promise = std::make_shared<std::promise<core::operations::lookup_in_any_replica_response>>();
        auto f = promise->get_future();
        cluster->execute(req, [promise](auto&& resp) {
            promise->set_value(std::forward<decltype(resp)>(resp));
        });
...
cb_Backend_document_lookup_in_all_replicas(VALUE self, VALUE bucket, VALUE scope, VALUE collection, VALUE id, VALUE specs, VALUE options)
{
    ...
    try {
        ...
        auto promise = std::make_shared<std::promise<core::operations::lookup_in_all_replicas_response>>();
        auto f = promise->get_future();
        cluster->execute(req, [promise](auto&& resp) {
            promise->set_value(std::forward<decltype(resp)>(resp));
        });
...
init_crud(VALUE cBackend)
{
    ...
    rb_define_method(cBackend, "document_lookup_in_any_replica", cb_Backend_document_lookup_in_any_replica, 6);
    rb_define_method(cBackend, "document_lookup_in_all_replicas", cb_Backend_document_lookup_in_all_replicas, 6);
