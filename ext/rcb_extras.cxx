...
cb_Backend_collections_manifest_get(VALUE self, VALUE bucket_name, VALUE timeout)
{
  ...
  try {
    ...
    if (resp.ctx.ec()) {
      cb_throw_error(
        resp.ctx,
        fmt::format("unable to get collections manifest of the bucket \"{}\"", req.id.bucket()));
    }
...
cb_Backend_dns_srv(VALUE self, VALUE hostname, VALUE service)
{
  ...
  try {
    ...
    auto resp = cb_wait_for_future(f);
    if (resp.ec) {
      cb_throw_error_code(resp.ec,
                          fmt::format("DNS SRV query failure for name \"{}\" (service: {})",
                                      host_name,
                                      service_name));
    }
...
cb_Backend_parse_connection_string(VALUE self, VALUE connection_string)
{
  ...
  std::string input(RSTRING_PTR(connection_string),
                    static_cast<std::size_t>(RSTRING_LEN(connection_string)));
  ...
  if (connstr.default_bucket_name) {
    rb_hash_aset(res,
                 ...
                 cb_str_new(connstr.default_bucket_name.value()));
  }
...
cb_Backend_snappy_compress(VALUE self, VALUE data)
{
  ...
  std::size_t compressed_size =
    snappy::Compress(RSTRING_PTR(data), static_cast<std::size_t>(RSTRING_LEN(data)), &compressed);
...
cb_Backend_snappy_uncompress(VALUE self, VALUE data)
{
  ...
  if (bool success = snappy::Uncompress(
        RSTRING_PTR(data), static_cast<std::size_t>(RSTRING_LEN(data)), &uncompressed);
      success) {
...
cb_Backend_leb128_decode(VALUE self, VALUE data)
{
  ...
  auto [value, rest] =
    core::utils::decode_unsigned_leb128<std::uint64_t>(buf, core::utils::leb_128_no_throw());
