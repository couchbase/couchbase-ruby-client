...
cb_Backend_document_get_multi(VALUE self, VALUE keys, VALUE options)
{
    ...
    try {
        ...
        for (const auto& promise : promises) {
            ...
            if (resp.ctx.ec()) {
                rb_hash_aset(entry, rb_id2sym(rb_intern("error")), cb_map_error(resp.ctx, "unable to (multi)fetch document"));
            }
...
cb_Backend_document_upsert_multi(VALUE self, VALUE bucket, VALUE scope, VALUE collection, VALUE id_content, VALUE options)
{
    ...
    try {
        ...
        for (auto& f : futures) {
            ...
            if (ctx.ec()) {
                rb_hash_aset(entry, rb_id2sym(rb_intern("error")), cb_map_error(ctx, "unable (multi)upsert"));
            }
...
cb_Backend_document_remove_multi(VALUE self, VALUE bucket, VALUE scope, VALUE collection, VALUE id_cas, VALUE options)
{
    ...
    try {
        ...
        for (auto& f : futures) {
            ...
            VALUE entry = to_mutation_result_value(resp);
            if (ctx.ec()) {
                rb_hash_aset(entry, rb_id2sym(rb_intern("error")), cb_map_error(ctx, "unable (multi)remove"));
            }
            rb_hash_aset(entry, rb_id2sym(rb_intern("id")), cb_str_new(ctx.id()));
            rb_ary_push(res, entry);
        }
