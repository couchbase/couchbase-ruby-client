...
cb_Backend_document_scan_create(VALUE self, VALUE bucket, VALUE scope, VALUE collection, VALUE scan_type, VALUE options)
{
    ...
    try {
        ...
        if (!agent.has_value()) {
            rb_raise(exc_couchbase_error(), "Cannot perform scan operation. Unable to get operation agent");
            return Qnil;
        }
        ...
        if (!config.has_value()) {
            rb_raise(exc_couchbase_error(), "Cannot perform scan operation. Unable to get bucket configuration");
            return Qnil;
        }
