...
cb_Backend_search_index_get_all(VALUE self, VALUE bucket, VALUE scope, VALUE options)
{
    ...
    try {
        ...
        auto promise = std::make_shared<std::promise<core::operations::management::search_index_get_all_response>>();
...
cb_Backend_search_index_get(VALUE self, VALUE bucket, VALUE scope, VALUE index_name, VALUE timeout)
{
    ...
    try {
        ...
        auto promise = std::make_shared<std::promise<core::operations::management::search_index_get_response>>();
        auto f = promise->get_future();
        cluster->execute(req, [promise](auto&& resp) {
            promise->set_value(std::forward<decltype(resp)>(resp));
        });
        ...
        if (resp.ctx.ec) {
            if (resp.error.empty()) {
                ...
                cb_throw_error(resp.ctx, fmt::format("unable to get search index \"{}\": {}", req.index_name, resp.error));
            }
        }
        VALUE res = rb_hash_new();
...
cb_Backend_search_index_upsert(VALUE self, VALUE bucket, VALUE scope, VALUE index_definition, VALUE timeout)
{
    ...
    try {
        ...
        auto promise = std::make_shared<std::promise<core::operations::management::search_index_upsert_response>>();
        auto f = promise->get_future();
        cluster->execute(req, [promise](auto&& resp) {
            promise->set_value(std::forward<decltype(resp)>(resp));
        });
        ...
        if (resp.ctx.ec) {
            if (resp.error.empty()) {
                cb_throw_error(resp.ctx, fmt::format("unable to upsert the search index \"{}\"", req.index.name));
            } else {
                cb_throw_error(resp.ctx, fmt::format("unable to upsert the search index \"{}\": {}", req.index.name, resp.error));
            }
        }
        VALUE res = rb_hash_new();
        rb_hash_aset(res, rb_id2sym(rb_intern("status")), cb_str_new(resp.status));
...
cb_Backend_search_index_drop(VALUE self, VALUE bucket, VALUE scope, VALUE index_name, VALUE timeout)
{
    ...
    try {
        ...
        cb_extract_timeout(req, timeout);
        req.index_name = cb_string_new(index_name);
        auto promise = std::make_shared<std::promise<core::operations::management::search_index_drop_response>>();
        auto f = promise->get_future();
        cluster->execute(req, [promise](auto&& resp) {
            promise->set_value(std::forward<decltype(resp)>(resp));
        });
        ...
        if (resp.ctx.ec) {
            if (resp.error.empty()) {
                cb_throw_error(resp.ctx, fmt::format("unable to drop the search index \"{}\"", req.index_name));
            } else {
                cb_throw_error(resp.ctx, fmt::format("unable to drop the search index \"{}\": {}", req.index_name, resp.error));
            }
        }
        VALUE res = rb_hash_new();
        rb_hash_aset(res, rb_id2sym(rb_intern("status")), cb_str_new(resp.status));
...
cb_Backend_search_index_get_documents_count(VALUE self, VALUE bucket, VALUE scope, VALUE index_name, VALUE timeout)
{
    ...
    try {
        ...
        cb_extract_timeout(req, timeout);
        req.index_name = cb_string_new(index_name);
        auto promise = std::make_shared<std::promise<core::operations::management::search_index_get_documents_count_response>>();
        auto f = promise->get_future();
        cluster->execute(req, [promise](auto&& resp) {
            promise->set_value(std::forward<decltype(resp)>(resp));
        });
        ...
        if (resp.ctx.ec) {
            if (resp.error.empty()) {
                cb_throw_error(resp.ctx,
                               fmt::format("unable to get number of the indexed documents for the search index \"{}\"", req.index_name));
            } else {
                cb_throw_error(
                  resp.ctx,
...
cb_Backend_search_index_get_stats(VALUE self, VALUE index_name, VALUE timeout)
{
    ...
    try {
        ...
        cb_extract_timeout(req, timeout);
        req.index_name = cb_string_new(index_name);
        auto promise = std::make_shared<std::promise<core::operations::management::search_index_get_stats_response>>();
        auto f = promise->get_future();
        cluster->execute(req, [promise](auto&& resp) {
            promise->set_value(std::forward<decltype(resp)>(resp));
        });
        ...
        if (resp.ctx.ec) {
            if (resp.error.empty()) {
                cb_throw_error(resp.ctx, fmt::format("unable to get stats for the search index \"{}\"", req.index_name));
            } else {
                cb_throw_error(resp.ctx, fmt::format("unable to get stats for the search index \"{}\": {}", req.index_name, resp.error));
            }
        }
...
cb_Backend_search_get_stats(VALUE self, VALUE timeout)
{
    ...
    try {
        ...
        cb_extract_timeout(req, timeout);
        auto promise = std::make_shared<std::promise<core::operations::management::search_get_stats_response>>();
        auto f = promise->get_future();
        cluster->execute(req, [promise](auto&& resp) {
            promise->set_value(std::forward<decltype(resp)>(resp));
        });
...
cb_Backend_search_index_pause_ingest(VALUE self, VALUE bucket, VALUE scope, VALUE index_name, VALUE timeout)
{
    ...
    try {
        ...
        req.index_name = cb_string_new(index_name);
        req.pause = true;
        auto promise = std::make_shared<std::promise<core::operations::management::search_index_control_ingest_response>>();
        auto f = promise->get_future();
        cluster->execute(req, [promise](auto&& resp) {
            promise->set_value(std::forward<decltype(resp)>(resp));
        });
        ...
        if (resp.ctx.ec) {
            if (resp.error.empty()) {
                cb_throw_error(resp.ctx, fmt::format("unable to pause ingest for the search index \"{}\"", req.index_name));
            } else {
                cb_throw_error(resp.ctx, fmt::format("unable to pause ingest for the search index \"{}\": {}", req.index_name, resp.error));
            }
        }
        VALUE res = rb_hash_new();
        rb_hash_aset(res, rb_id2sym(rb_intern("status")), cb_str_new(resp.status));
...
cb_Backend_search_index_resume_ingest(VALUE self, VALUE bucket, VALUE scope, VALUE index_name, VALUE timeout)
{
    ...
    try {
        ...
        if (resp.ctx.ec) {
            if (resp.error.empty()) {
                cb_throw_error(resp.ctx, fmt::format("unable to resume ingest for the search index \"{}\"", req.index_name));
            } else {
                cb_throw_error(resp.ctx,
...
cb_Backend_search_index_allow_querying(VALUE self, VALUE bucket, VALUE scope, VALUE index_name, VALUE timeout)
{
    ...
    try {
        ...
        req.index_name = cb_string_new(index_name);
        req.allow = true;
        auto promise = std::make_shared<std::promise<core::operations::management::search_index_control_query_response>>();
        auto f = promise->get_future();
        cluster->execute(req, [promise](auto&& resp) {
            promise->set_value(std::forward<decltype(resp)>(resp));
        });
        ...
        if (resp.ctx.ec) {
            if (resp.error.empty()) {
                cb_throw_error(resp.ctx, fmt::format("unable to allow querying for the search index \"{}\"", req.index_name));
            } else {
                cb_throw_error(resp.ctx,
...
cb_Backend_search_index_disallow_querying(VALUE self, VALUE bucket, VALUE scope, VALUE index_name, VALUE timeout)
{
    ...
    try {
        ...
        if (resp.ctx.ec) {
            if (resp.error.empty()) {
                cb_throw_error(resp.ctx, fmt::format("unable to disallow querying for the search index \"{}\"", req.index_name));
            } else {
                cb_throw_error(resp.ctx,
...
cb_Backend_search_index_freeze_plan(VALUE self, VALUE bucket, VALUE scope, VALUE index_name, VALUE timeout)
{
    ...
    try {
        ...
        req.index_name = cb_string_new(index_name);
        req.freeze = true;
        auto promise = std::make_shared<std::promise<core::operations::management::search_index_control_plan_freeze_response>>();
        auto f = promise->get_future();
        cluster->execute(req, [promise](auto&& resp) {
            promise->set_value(std::forward<decltype(resp)>(resp));
        });
        ...
        if (resp.ctx.ec) {
            if (resp.error.empty()) {
                cb_throw_error(resp.ctx, fmt::format("unable to freeze for the search index \"{}\"", req.index_name));
            } else {
                cb_throw_error(resp.ctx, fmt::format("unable to freeze for the search index \"{}\": {}", req.index_name, resp.error));
            }
        }
        VALUE res = rb_hash_new();
        rb_hash_aset(res, rb_id2sym(rb_intern("status")), cb_str_new(resp.status));
...
cb_Backend_search_index_unfreeze_plan(VALUE self, VALUE bucket, VALUE scope, VALUE index_name, VALUE timeout)
{
    ...
    try {
        ...
        if (resp.ctx.ec) {
            if (resp.error.empty()) {
                cb_throw_error(resp.ctx, fmt::format("unable to unfreeze plan for the search index \"{}\"", req.index_name));
            } else {
                cb_throw_error(resp.ctx, fmt::format("unable to unfreeze for the search index \"{}\": {}", req.index_name, resp.error));
            }
        }
        VALUE res = rb_hash_new();
        rb_hash_aset(res, rb_id2sym(rb_intern("status")), cb_str_new(resp.status));
...
cb_Backend_search_index_analyze_document(VALUE self, VALUE bucket, VALUE scope, VALUE index_name, VALUE encoded_document, VALUE timeout)
{
    ...
    try {
        ...
        auto promise = std::make_shared<std::promise<core::operations::management::search_index_analyze_document_response>>();
        auto f = promise->get_future();
        cluster->execute(req, [promise](auto&& resp) {
            promise->set_value(std::forward<decltype(resp)>(resp));
        });
        ...
        if (resp.ctx.ec) {
            if (resp.error.empty()) {
                cb_throw_error(resp.ctx, fmt::format("unable to analyze document using the search index \"{}\"", req.index_name));
            } else {
                cb_throw_error(resp.ctx,
                               fmt::format("unable to analyze document using the search index \"{}\": {}", req.index_name, resp.error));
            }
        }
        VALUE res = rb_hash_new();
        rb_hash_aset(res, rb_id2sym(rb_intern("status")), cb_str_new(resp.status));
...
cb_Backend_document_search(VALUE self, VALUE bucket, VALUE scope, VALUE index_name, VALUE query, VALUE search_request, VALUE options)
{
    ...
    try {
        ...
        auto resp = cb_wait_for_future(f);
        if (resp.ctx.ec) {
            cb_throw_error(resp.ctx, fmt::format("unable to perform search query for index \"{}\": {}", req.index_name, resp.error));
        }
        VALUE res = rb_hash_new();
...
init_search(VALUE cBackend)
{
    ...
    rb_define_method(cBackend, "search_index_get_documents_count", cb_Backend_search_index_get_documents_count, 4);
    rb_define_method(cBackend, "search_index_pause_ingest", cb_Backend_search_index_pause_ingest, 4);
    rb_define_method(cBackend, "search_index_resume_ingest", cb_Backend_search_index_resume_ingest, 4);
    rb_define_method(cBackend, "search_index_allow_querying", cb_Backend_search_index_allow_querying, 4);
    rb_define_method(cBackend, "search_index_disallow_querying", cb_Backend_search_index_disallow_querying, 4);
    rb_define_method(cBackend, "search_index_freeze_plan", cb_Backend_search_index_freeze_plan, 4);
    rb_define_method(cBackend, "search_index_unfreeze_plan", cb_Backend_search_index_unfreeze_plan, 4);
    rb_define_method(cBackend, "search_index_analyze_document", cb_Backend_search_index_analyze_document, 5);
}
} // namespace couchbase::ruby
