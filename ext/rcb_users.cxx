...
cb_Backend_role_get_all(VALUE self, VALUE timeout)
{
    ...
    try {
        ...
        auto promise = std::make_shared<std::promise<core::operations::management::role_get_all_response>>();
...
cb_Backend_user_get_all(VALUE self, VALUE domain, VALUE timeout)
{
    ...
    try {
        ...
        if (domain == rb_id2sym(rb_intern("local"))) {
            ...
            throw ruby_exception(exc_invalid_argument(), rb_sprintf("unsupported authentication domain: %+" PRIsVALUE, domain));
        }
        auto promise = std::make_shared<std::promise<core::operations::management::user_get_all_response>>();
        auto f = promise->get_future();
        cluster->execute(req, [promise](auto&& resp) {
            promise->set_value(std::forward<decltype(resp)>(resp));
        });
...
cb_Backend_user_get(VALUE self, VALUE domain, VALUE username, VALUE timeout)
{
    ...
    try {
        ...
        auto promise = std::make_shared<std::promise<core::operations::management::user_get_response>>();
        auto f = promise->get_future();
        cluster->execute(req, [promise](auto&& resp) {
            promise->set_value(std::forward<decltype(resp)>(resp));
        });
...
cb_Backend_user_drop(VALUE self, VALUE domain, VALUE username, VALUE timeout)
{
    ...
    try {
        ...
        req.username = cb_string_new(username);
        auto promise = std::make_shared<std::promise<core::operations::management::user_drop_response>>();
        auto f = promise->get_future();
        cluster->execute(req, [promise](auto&& resp) {
            promise->set_value(std::forward<decltype(resp)>(resp));
        });
...
cb_Backend_user_upsert(VALUE self, VALUE domain, VALUE user, VALUE timeout)
{
    ...
    try {
        ...
        auto promise = std::make_shared<std::promise<core::operations::management::user_upsert_response>>();
        auto f = promise->get_future();
        cluster->execute(req, [promise](auto&& resp) {
            promise->set_value(std::forward<decltype(resp)>(resp));
        });
        ...
        if (auto resp = cb_wait_for_future(f); resp.ctx.ec) {
            cb_throw_error(resp.ctx, fmt::format(R"(unable to upsert user "{}" ({}))", req.user.username, fmt::join(resp.errors, ", ")));
        }
...
cb_Backend_change_password(VALUE self, VALUE new_password, VALUE timeout)
{
    ...
    try {
        ...
        auto promise = std::make_shared<std::promise<core::operations::management::change_password_response>>();
        auto f = promise->get_future();
        cluster->execute(req, [promise](auto&& resp) {
            promise->set_value(std::forward<decltype(resp)>(resp));
        });
...
cb_Backend_group_get_all(VALUE self, VALUE timeout)
{
    ...
    try {
        ...
        cb_extract_timeout(req, timeout);
        auto promise = std::make_shared<std::promise<core::operations::management::group_get_all_response>>();
        auto f = promise->get_future();
        cluster->execute(req, [promise](auto&& resp) {
            promise->set_value(std::forward<decltype(resp)>(resp));
        });
...
cb_Backend_group_get(VALUE self, VALUE name, VALUE timeout)
{
    ...
    try {
        ...
        auto promise = std::make_shared<std::promise<core::operations::management::group_get_response>>();
        auto f = promise->get_future();
        cluster->execute(req, [promise](auto&& resp) {
            promise->set_value(std::forward<decltype(resp)>(resp));
        });
...
cb_Backend_group_drop(VALUE self, VALUE name, VALUE timeout)
{
    ...
    try {
        ...
        cb_extract_timeout(req, timeout);
        req.name = cb_string_new(name);
        auto promise = std::make_shared<std::promise<core::operations::management::group_drop_response>>();
        auto f = promise->get_future();
        cluster->execute(req, [promise](auto&& resp) {
            promise->set_value(std::forward<decltype(resp)>(resp));
        });
...
cb_Backend_group_upsert(VALUE self, VALUE group, VALUE timeout)
{
    ...
    try {
        ...
        auto promise = std::make_shared<std::promise<core::operations::management::group_upsert_response>>();
        auto f = promise->get_future();
        cluster->execute(req, [promise](auto&& resp) {
            promise->set_value(std::forward<decltype(resp)>(resp));
        });
        if (auto resp = cb_wait_for_future(f); resp.ctx.ec) {
            cb_throw_error(resp.ctx, fmt::format(R"(unable to upsert group "{}" ({}))", req.group.name, fmt::join(resp.errors, ", ")));
        }
        return Qtrue;
    } catch (const std::system_error& se) {
        rb_exc_raise(cb_map_error_code(se.code(), fmt::format("failed to perform {}: {}", __func__, se.what()), false));
