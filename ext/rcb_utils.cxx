...
extract_legacy_durability_replicate_to(VALUE options)
{
    ...
    if (VALUE val = rb_hash_aref(options, property_name); !NIL_P(val)) {
        ...
        throw ruby_exception(exc_invalid_argument(), rb_sprintf("unknown replicate_to: %+" PRIsVALUE, val));
    }
...
extract_legacy_durability_persist_to(VALUE options)
{
    ...
    if (VALUE val = rb_hash_aref(options, property_name); !NIL_P(val)) {
        ...
        throw ruby_exception(exc_invalid_argument(), rb_sprintf("unknown persist_to value: %+" PRIsVALUE, val));
    }
...
{
    return core::utils::to_binary(static_cast<const char*>(RSTRING_PTR(str)), static_cast<std::size_t>(RSTRING_LEN(str)));
}
...
cb_extract_option_symbol(VALUE& val, VALUE options, const char* name)
{
    if (!NIL_P(options) && TYPE(options) == T_HASH) {
        ...
        throw couchbase::ruby::ruby_exception(rb_eArgError, rb_sprintf("%s must be an Symbol, but given %+" PRIsVALUE, name, val));
    }
}
...
cb_extract_option_string(VALUE& val, VALUE options, const char* name)
{
    if (!NIL_P(options) && TYPE(options) == T_HASH) {
        ...
        throw couchbase::ruby::ruby_exception(rb_eArgError, rb_sprintf("%s must be an String, but given %+" PRIsVALUE, name, val));
    }
}
...
cb_extract_option_bignum(VALUE& val, VALUE options, const char* name)
{
    if (!NIL_P(options) && TYPE(options) == T_HASH) {
        ...
        throw couchbase::ruby::ruby_exception(rb_eArgError, rb_sprintf("%s must be an Integer, but given %+" PRIsVALUE, name, val));
    }
}
...
cb_extract_option_array(VALUE& val, VALUE options, const char* name)
{
    if (!NIL_P(options) && TYPE(options) == T_HASH) {
        ...
        throw couchbase::ruby::ruby_exception(rb_eArgError, rb_sprintf("%s must be an Array, but given %+" PRIsVALUE, name, val));
    }
}
...
cb_extract_cas(couchbase::cas& field, VALUE cas)
{
    switch (TYPE(cas)) {
        ...
        case T_BIGNUM:
            ...
            break;
        default:
            throw couchbase::ruby::ruby_exception(rb_eArgError, rb_sprintf("CAS must be an Integer, but given %+" PRIsVALUE, cas));
    }
}
...
extract_durability_level(VALUE options)
{
    ...
    if (VALUE val = rb_hash_aref(options, property_name); !NIL_P(val)) {
        ...
        throw ruby_exception(exc_invalid_argument(), rb_sprintf("unknown durability level: %+" PRIsVALUE, val));
    }
