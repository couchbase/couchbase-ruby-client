...
#if defined(HAVE_RUBY_VERSION_H)
    user_agent_value.append(fmt::format(";ruby_abi/{}.{}.{}",
...
init_version(VALUE mCouchbase)
{
  ...
  rb_hash_aset(
    ...
    rb_str_freeze(cb_str_new(fmt::format(
      "{}.{}.{}", RUBY_API_VERSION_MAJOR, RUBY_API_VERSION_MINOR, RUBY_API_VERSION_TEENY))));
#endif
  rb_hash_aset(
    cb_Version, rb_id2sym(rb_intern("revision")), rb_str_freeze(rb_str_new_cstr(EXT_GIT_REVISION)));

  VALUE version_info = rb_inspect(cb_Version);
  CB_LOG_DEBUG("couchbase backend has been initialized: {}",
               std::string_view(RSTRING_PTR(version_info),
                                static_cast<std::size_t>(RSTRING_LEN(version_info))));
  ...
  rb_hash_aset(
    ...
    rb_str_freeze(cb_str_new(fmt::format(
      "{}.{}.{}", RUBY_API_VERSION_MAJOR, RUBY_API_VERSION_MINOR, RUBY_API_VERSION_TEENY))));
#endif
  rb_hash_aset(cb_BuildInfo,
               rb_id2sym(rb_intern("revision")),
               rb_str_freeze(rb_str_new_cstr(EXT_GIT_REVISION)));
  rb_hash_aset(cb_BuildInfo,
               rb_id2sym(rb_intern("ruby_librubyarg")),
               rb_str_freeze(rb_str_new_cstr(RUBY_LIBRUBYARG)));
  rb_hash_aset(cb_BuildInfo,
               rb_id2sym(rb_intern("ruby_include_dir")),
               rb_str_freeze(rb_str_new_cstr(RUBY_INCLUDE_DIR)));
  rb_hash_aset(cb_BuildInfo,
               rb_id2sym(rb_intern("ruby_library_dir")),
               rb_str_freeze(rb_str_new_cstr(RUBY_LIBRARY_DIR)));
  const auto user_agent = user_agent_with_extra();
  rb_hash_aset(
    cb_BuildInfo, rb_id2sym(rb_intern("user_agent")), rb_str_freeze(cb_str_new(user_agent)));
  ...
  for (const auto& [name, value] : core::meta::sdk_build_info()) {
    if (name == "version_major" || name == "version_minor" || name == "version_patch" ||
      ...
      rb_hash_aset(cb_CoreInfo,
                   rb_id2sym(rb_intern(name.c_str())),
                   rb_str_freeze(rb_str_new_cstr(value.c_str())));
    }
  }
  rb_hash_aset(cb_BuildInfo, rb_id2sym(rb_intern("cxx_client")), cb_CoreInfo);
  VALUE build_info = rb_inspect(cb_BuildInfo);
  CB_LOG_DEBUG(
    "couchbase backend build info: {}",
