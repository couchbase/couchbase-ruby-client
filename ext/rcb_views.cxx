...
cb_Backend_view_index_get_all(VALUE self, VALUE bucket_name, VALUE name_space, VALUE timeout)
{
    ...
    try {
        ...
        auto promise = std::make_shared<std::promise<core::operations::management::view_index_get_all_response>>();
...
cb_Backend_view_index_get(VALUE self, VALUE bucket_name, VALUE document_name, VALUE name_space, VALUE timeout)
{
    ...
    try {
        ...
        req.ns = ns;
        cb_extract_timeout(req, timeout);
        auto promise = std::make_shared<std::promise<core::operations::management::view_index_get_response>>();
        auto f = promise->get_future();
        cluster->execute(req, [promise](auto&& resp) {
            promise->set_value(std::forward<decltype(resp)>(resp));
        });
...
cb_Backend_view_index_drop(VALUE self, VALUE bucket_name, VALUE document_name, VALUE name_space, VALUE timeout)
{
    ...
    try {
        ...
        req.ns = ns;
        cb_extract_timeout(req, timeout);
        auto promise = std::make_shared<std::promise<core::operations::management::view_index_drop_response>>();
        auto f = promise->get_future();
        cluster->execute(req, [promise](auto&& resp) {
            promise->set_value(std::forward<decltype(resp)>(resp));
        });
...
cb_Backend_view_index_upsert(VALUE self, VALUE bucket_name, VALUE document, VALUE name_space, VALUE timeout)
{
    ...
    try {
        ...
        cb_extract_timeout(req, timeout);
        auto promise = std::make_shared<std::promise<core::operations::management::view_index_upsert_response>>();
        auto f = promise->get_future();
        cluster->execute(req, [promise](auto&& resp) {
            promise->set_value(std::forward<decltype(resp)>(resp));
        });
...
cb_Backend_document_view(VALUE self, VALUE bucket_name, VALUE design_document_name, VALUE view_name, VALUE name_space, VALUE options)
{
    ...
    try {
        ...
        if (resp.ctx.ec) {
            if (resp.error) {
                cb_throw_error(resp.ctx, fmt::format(R"(unable to execute view query {} ({}))", resp.error->code, resp.error->message));
            } else {
