    ...
    module Commands
      module QueryIndexManager
        class CreateIndexCommand
          def initialize(manager:, index_name:, fields:, initiated:, get_span_fn:, return_result: true, raw_options: nil, bucket_name: nil)
            @manager = manager
            @raw_options = raw_options
            @cmd_args = bucket_name.nil? ? [] : [bucket_name]
            @cmd_args.append(index_name, fields)
            @initiated = initiated
          ...
          def set_options
            ...
            builder = OptionsBuilder.new(
              options: Couchbase::Management::Options::Query::CreateIndex.new,
              raw_options: @raw_options,
            )
            builder.set_ignore_if_exists
                   .set_num_replicas
                   .set_deferred
                   .set_scope_name
                   .set_collection_name
          ...
          def execute_command
            Results.as_success(initiated: @initiated) do
              @manager.create_index(*@cmd_args)
            end
          end

          def self.create_command(...)
            cmd = CreateIndexCommand.new(...)
            cmd.set_options
            cmd
          end
        end
