    ...
    module Commands
      module QueryIndexManager
        class DropIndexCommand
          def initialize(manager:, index_name:, initiated:, get_span_fn:, return_result: true, raw_options: nil, bucket_name: nil)
            @manager = manager
            @raw_options = raw_options
            @cmd_args = bucket_name.nil? ? [] : [bucket_name]
            @cmd_args.append(index_name)
            @initiated = initiated
          ...
          def set_options
            ...
            builder = OptionsBuilder.new(
              options: Couchbase::Management::Options::Query::DropIndex.new,
              raw_options: @raw_options,
            )
            builder.set_ignore_if_does_not_exist
                   .set_scope_name
                   .set_collection_name
                   .set_timeout
          ...
          def execute_command
            Results.as_success(initiated: @initiated) do
              @manager.drop_index(*@cmd_args)
            end
          end

          def self.create_command(...)
            cmd = DropIndexCommand.new(...)
            cmd.set_options
            cmd
          end
        end
