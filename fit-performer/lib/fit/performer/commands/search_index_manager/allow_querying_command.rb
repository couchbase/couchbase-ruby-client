        ...
        class AllowQueryingCommand
          ...
          def set_options
            ...
            builder = OptionsBuilder.new(
              options: Couchbase::Management::SearchIndexManager::AllowQueryingOptions.new,
              raw_options: @raw_options,
            )
            builder.set_timeout
          ...
          def execute_command
            Results.as_success(initiated: @initiated) do
              @manager.allow_querying(*@cmd_args)
            end
          end

          def self.create_command(...)
            cmd = AllowQueryingCommand.new(...)
            cmd.set_options
            cmd
          end
        end
