        ...
        class UnfreezePlanCommand
          ...
          def set_options
            ...
            builder = OptionsBuilder.new(
              options: Couchbase::Management::SearchIndexManager::UnfreezePlanOptions.new,
              raw_options: @raw_options,
            )
            builder.set_timeout
          ...
          def execute_command
            Results.as_success(initiated: @initiated) do
              @manager.unfreeze_plan(*@cmd_args)
            end
          end

          def self.create_command(...)
            cmd = UnfreezePlanCommand.new(...)
            cmd.set_options
            cmd
          end
        end
