...
module Couchbase
  class CrudTest < Minitest::Test
    ...
    def test_projection_few_paths
      ...
      test_cases = [
        ...
        {name: "array entries", project: %w[animals[1] animals[0]],
         expected: {"animals" => env.protostellar? ? [person["animals"][0], person["animals"][1]] : [person["animals"][1], person["animals"][0]]}},
      ]
    ...
    def test_collection_retry
      skip("#{name}: This test cannot be run with the #{Couchbase::Protostellar::NAME} protocol because of the use of #__wait_for_collections_manifest") if env.protostellar?
